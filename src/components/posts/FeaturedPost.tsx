        ...
        <div>
          <div className="flex gap-2 flex-wrap">
            {post.categories?.map((category, key) => (
              <p
                key={key}
                className="bg-greenAccent p-1 text-slate-100 rounded-md"
              >
                {category.toLocaleString()}
              </p>
            ))}
          </div>
          <h3 className="font-bold text-2xl mt-4 lg:text-4xl lg:mt-6 hover:text-greenAccent transition-colors w-fit">
